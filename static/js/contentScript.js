...
function setUsernameset() {
  chrome.storage.sync.set({ 'usernameset': 1 }, function() {
        ...
        if (tweetArea) {
          try {
            ...
            chrome.storage.sync.get("usernameset", function(result) {
              let pasteText;
              if (result.usernameset && result.usernameset === 1) { // Display displayName only if usernameset is set to 1
                pasteText = `${gmMessage} ${displayName}`;
              } else { // Otherwise, do not display displayName
                pasteText = `${gmMessage}`;
              }
      ...
      if (tweetArea) {
        try {
          ...
          chrome.storage.sync.get("usernameset", function(result) {
            let pasteText;
            if (result.usernameset && result.usernameset === 1) { // Display displayName only if usernameset is set to 1
              pasteText = `${gnMessage} ${displayName}`;
            } else { // Otherwise, do not display displayName
              pasteText = `${gnMessage}`;
            }
